...
/* ── Streamit Hero Slider ─────────────────────────────────── */
(function () {
  const slider = document.querySelector('[data-streamit-slider]');
  if (!slider) return;

  const slides = Array.from(slider.querySelectorAll('.streamit-hero__slide'));
  ...
  const prevBtn = slider.querySelector('[data-streamit-prev]');
  const nextBtn = slider.querySelector('[data-streamit-next]');
